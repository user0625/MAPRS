...
当前实现已支持持久化多轮会话、最近上下文追问改写、BM25/向量混合检索与 RRF、章节筛选、中英文回答、Evidence 回溯、引用白名单、可恢复流式事件、取消和失败重试。Mock Embedding、模型改写失败或 Embedding 服务异常均可安全降级。多论文对话、页码范围筛选和更大规模的问答评估仍属于后续范围。
...
> 状态更新（2026-07-12）：单篇论文的对话式持续问答已在 MVP 后续迭代中完成；其余条目仍未实现。
...
- Ask-the-Paper Q&A 深化：页码范围、会话管理、评估集与多论文扩展
