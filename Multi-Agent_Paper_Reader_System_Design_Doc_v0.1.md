...
当前实现已支持持久化多轮会话、当前论文内会话搜索、事务级单会话删除、Markdown/JSON 引用归档、最近上下文追问改写、BM25/向量混合检索与 RRF、章节筛选、中英文回答、Evidence 回溯、引用白名单、可恢复流式事件、取消和失败重试。Mock Embedding、模型改写失败或 Embedding 服务异常均可安全降级。多论文对话、页码范围筛选、更精细的上下文窗口管理和更大规模的问答评估仍属于后续范围。
...
> 状态更新（2026-07-13）：单篇论文的对话式持续问答及会话搜索、删除、引用归档已完成；下一产品阶段为页码范围和上下文窗口管理，其余条目仍未实现。
...
- Ask-the-Paper Q&A 深化：页码范围、上下文窗口管理、评估集与多论文扩展
