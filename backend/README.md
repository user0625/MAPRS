...
`/api/comparisons` 可基于 2–5 个唯一的已完成分析任务创建持久比较。系统会快照
源论文元数据与 Evidence；完成后即使删除源任务，比较产物仍可独立读取。活动
比较会阻止源任务删除，事件流可通过
`GET /api/comparisons/{id}/events?after=<sequence>` 断点恢复。

`paper-comparison-v1` 包含有序源论文、逐论文 profile、七维矩阵、综合 claims、
比较命名空间 Evidence ID、预算信息和质量警告，并提供 Markdown、JSON、HTML、
PDF、DOCX 五种附件。
...
- 为已完成任务提供同步只读 Document Search，支持 Auto/BM25、章节/页码、Top-K、相邻 chunk、分数解释、持久索引复用和安全降级。
- 为 2–5 个历史完成任务提供持久化多论文 profile、七维矩阵、综合报告、跨论文 Evidence 快照、可恢复事件流和五格式导出。
...
| `COMPARISON_PAPER_MAX_TOKENS` | `6000` | 单篇论文比较输入预算 |
| `COMPARISON_FINAL_MAX_TOKENS` | `12000` | 最终综合预算 |
| `COMPARISON_EVIDENCE_PER_PAPER` | `12` | 单篇论文最多保留的比较 Evidence |
...
真实私有评估基础设施已完成：仓库提供脱敏 Schema/fixture、候选生成、validation 校准、test 冻结保护和单次质量门。`pilot-only-v3` 代码已准备，但真实 embedding/reranker preflight 当前均超时，因此未启动完整 Pilot；最后保存的质量结论仍来自 v2。该流程作为生产化扩展保留，不阻塞当前项目继续完成文档搜索闭环；test 仍不访问，reranker 保持 `disabled`。完整字段、限制、CLI 和指标口径见 [`evaluation/README.md`](evaluation/README.md)。
...
`POST /api/tasks/{task_id}/search` 只接受已完成且 state 有效的任务。请求支持 `query`、`auto | bm25`、可选章节、成对的 1-based 页码闭区间和 1–20 的 `top_k`。Auto 在 embedding 已配置时复用 Ask Paper 的进程内 LRU 与 `ask-retrieval-index-v1`，执行 BM25 + vector + RRF；Mock/未配置 embedding 直接使用 BM25，索引或查询失败返回 `degraded_to_bm25`。

响应不含向量、正文 state、密钥或完整上游错误；诊断只含实际模式、候选数、耗时、`memory_hit | disk_hit | cold_build | unavailable` 和脱敏降级类别。排名片段最多 1200 字符，直接相邻上下文最多 600 字符，且不跨命中章节或请求页码边界。该路径强制不调用 reranker、不持久化历史，也不创建 Evidence ID。
...
ASK_INDEX_PREBUILD_ENABLED=true
ASK_INDEX_DIR=backend/outputs/indexes
...
全文索引以 `ask-retrieval-index-v1` 原子写入磁盘，只保存 chunk ID、归一化向量、模型和内容摘要，不保存论文正文。Worker LRU 在内存中附加从 state 构建的 BM25 统计；章节和页码约束从同一全文索引切片。state、chunk、provider/model 或 Schema 变化会自动重建，损坏文件会被忽略，任务删除与保留期清理同步删除索引。所有进入模型上下文的检索 Evidence 均保存为消息快照，但 `citation_ids` 只记录答案实际使用且属于本次白名单的 ID；非法和跨消息引用会从最终答案清除。无候选证据时不会调用回答生成模型。诊断日志记录范围、预算用量、分支状态、降级类型、候选数和融合分数，不记录论文正文、完整问题或密钥。
...
比较对象固定为 2–5 个唯一、已完成且 state/report 可用的历史任务。创建返回 `202`，源论文顺序会保留到 profile、矩阵和导出：

```text
POST   /api/comparisons
GET    /api/comparisons?limit={limit}&offset={offset}&search={text}&status={status}
...
```json
{
  ...
  "title": "可选标题",
  "focus": "方法、实验结果与局限的综合比较",
  "language": "zh"
}
```

Worker 从各源 state 的 chunks 检索研究问题、数据集、方法、基线、指标、结果与局限，每篇最多保留 `COMPARISON_EVIDENCE_PER_PAPER` 条 Evidence，并在配置预算内生成 `paper-comparison-v1`。Evidence ID 使用 comparison 命名空间；结构化产物落盘前递归删除不在本次白名单内的非法或跨比较引用。

`paper_comparisons`、`comparison_papers`、`comparison_evidence` 和 `comparison_events` 由 `0004_paper_comparisons.py` 创建。活动比较会阻止源任务删除；终态后依靠元数据、Evidence 和报告快照独立保存，源任务可删除。比较删除会清理其全部快照、事件和五格式产物。首版不包含新文件上传、远程论文来源、跨论文聊天、协作编辑或权限系统。
...
Pilot 与多论文比较专项覆盖只读派生视图、test 禁止访问、脱敏报告、强制 disabled、2–5 篇约束、顺序快照、事件恢复、源删除保护、终态独立性、Evidence 命名空间、预算、引用清理和五种导出：

```bash
uv run pytest backend/tests/test_pilot_gate.py \
  backend/tests/test_comparisons.py \
  backend/tests/test_api_comparisons.py -q -rs
```
...
已完成的基础能力包括 React Web UI、PostgreSQL/SQLite 持久化、Redis + Celery 任务队列、可恢复 SSE、结构化报告与 Evidence、Document Search、单篇论文 Ask Paper，以及基于 2–5 个历史完成任务的多论文对比 MVP。后续工作按优先级为：

1. **演示完整性**：继续补充从上传、分析、搜索、问答到多论文对比的演示数据与浏览器回归。
2. **检索质量加分项**：在不影响离线 BM25 演示的前提下扩充正式人工 gold，并校准真实 embedding/reranker。
3. **文档理解加分项**：OCR、复杂多栏版面、表格、公式、图片与图注，并保留可引用的位置关系。
4. **多论文研究加分项**：批量/arXiv/DOI/URL 输入、跨论文聊天、研究空白与文献综述。
5. **生产化加分项**：正式人工 gold、Pilot/test、外部向量数据库、认证、配额、Metrics/Tracing、Secret 管理和数据库备份。
...
- 多论文对比首版只接受 2–5 个现有已完成任务，不负责上传或抓取新论文；综合内容由源 state chunks 支撑，已有单论文报告不代替带页码 Evidence。
...
- 分析工作流内部的 `NumpyVectorStore` 仍只存在进程内；Ask Paper 和 Search Document 复用 `ask-retrieval-index-v1` 磁盘索引。该单任务文件方案适合项目演示，不适合大规模语料库。
