...
真实私有评估的基础设施已完成：仓库提供脱敏 Schema/fixture，以及候选生成、production profile 校验、validation 阈值校准、test 内容冻结和单次冻结质量门。专家裁决中 122 条样本只允许作为 `pilot_only` 工程证据，其中 validation 为 20 条；最新一次真实 shadow 因当前 embedding/reranker 端点不可用而降级，仍有 7 项硬门失败，`exit_ready_for_comparison_mvp=false`。test 未访问或冻结，生产继续使用 BM25 且 reranker 保持 `disabled`。完整字段、限制、CLI 和指标口径见 [`evaluation/README.md`](evaluation/README.md)。
...
已完成的基础能力包括 React Web UI、PostgreSQL/SQLite 持久化、Redis + Celery 任务队列、可恢复 SSE、结构化报告与 Evidence、单篇论文 Ask Paper，以及基于 2–5 个历史完成任务的多论文对比 MVP。后续工作按优先级为：

1. **检索与问答质量**：会话管理、页码/章节范围和预算化上下文已完成；继续扩展正式人工 gold，并解决最新 pilot validation 的 7 项失败，在冻结测试通过前维持 BM25 与 reranker `disabled`。
2. **文档理解**：OCR、复杂多栏版面、表格、公式、图片与图注，并保留可引用的位置关系。
3. **多论文研究**：当前已有历史任务对比、Evidence 快照和矩阵；后续增加批量/arXiv/DOI/URL 输入、跨论文聊天、研究空白与文献综述。
