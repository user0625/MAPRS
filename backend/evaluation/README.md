...
Hybrid 选择先过滤通过正式 Retrieval 门的配置，再从中优先选择同时满足上述八项 validation 指标的 `pilot_validation_proxy` 配置；两层都沿用 Candidate Recall、Recall、Precision、MRR 的既有排序。若 proxy 子集为空，则安全回退 Retrieval 最优配置，不把它标记为 validation-ready。proxy 只用于 train Pilot 的配置选择，`validation_authorized` 仍只由正式 Retrieval 门决定。
...
正式 Retrieval 门通过并继续授权 `retrieval_only` validation。Reranker 与 Refusal 门仍失败，3267 组诊断配置中可行数仍为 0，`quality_gate.passed=false`，Reranker 推荐保持 `disabled`。v2 作为有效历史记录保留，但其 Evidence=6 配置未通过 Precision/F1 proxy；后续 validation 应只复用 v3。v3 已满足“Retrieval 门和所选配置 proxy 都通过”的启动条件，但本轮仍未运行 validation、test 或人工评估，也未把 Embedding 切换为默认配置。

建议的 retrieval-only validation 命令如下，但只有在明确启动下一阶段时才执行：
