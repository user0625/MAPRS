...
```bash
EMBEDDING_PROVIDER=mock ASK_RERANKER_MODEL= ASK_RERANKER_MODE=disabled \
uv run python -m backend.evaluation.pilot \
  ...
  backend/outputs/evaluation/pilot-only-v1.json pilot-only-v1
```
...
`pilot-only-v2` 的 embedding/reranker 显式降级路径均完成，但当前配置的真实端点在放宽网络后仍返回连接级错误，因此 shadow 降级为非 reranker 路径。报告尚有 7 项硬门失败：Candidate Recall@20、Recall@6、Precision@6、MRR、Evidence coverage、无答案拒答率和 p95 延迟；`exit_ready_for_comparison_mvp=false`。生产继续保持 `ASK_RERANKER_MODE=disabled`，不得输出或采纳生产启用建议。后续应先恢复可用模型端点、只在 analysis/validation 重跑离线校准，并扩充正式人工 gold；全部 validation 硬门通过后才能一次性冻结和运行 test。
...
合成夹具已完成 CI 回归职责，真实私有评估的 Schema、生成/校验/校准 CLI、论文级 split、test 冻结保护和硬质量门也已完成。Pilot 证明只读派生、脱敏报告和降级链路可用，但 validation 尚未达标，不能启用 reranker 或把 test 用于调参。下一步应恢复模型端点、扩充正式人工 gold、分析当前 7 项失败，并在 validation 全部通过后一次性冻结 test。多论文对比 MVP 的实现和测试不改变这一质量结论。
