...
`pilot-v1` 仅用于验证私有数据集、validation 校准和报告链路是否可运行，不是生产质量验收。当前共 122 条样本，其中 validation 20 条；test 尚未冻结，也未运行 frozen-test。validation 报告仍有 8 项硬门槛失败，reranker 未形成足够收益，因此 BM25 是当前最稳妥的检索基线，生产建议继续保持 `ASK_RERANKER_MODE=disabled`。后续应先补足约 200 条已复核样本并解决 validation 失败，再一次性冻结和运行 test。
...
合成夹具已完成 CI 回归职责，真实私有评估的 Schema、生成/校验/校准 CLI、论文级 split、test 冻结保护和硬质量门也已完成。`pilot-v1` 证明工程链路可用，但 validation 尚未达标，不能启用 reranker 或把 test 用于调参。下一步应扩充并复核样本、分析 8 项失败、在 validation 达标后一次性冻结 test；产品开发则进入 Ask Paper 会话搜索、删除和 Markdown/JSON 导出。
