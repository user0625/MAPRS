...
async function installFixtures(page: Page) {
  ...
  await page.route(/^https?:\/\/[^/]+\/api\//, async route => {
    ...
    if (path === '/api/tasks/task-1/conversations') return request.method() === 'POST'
      ? json({ id: 'conv-new', task_id: 'task-1', title: 'New conversation', language: 'auto', created_at: now, updated_at: now }, 201)
      : json({ items: [{ id: 'conv-1', task_id: 'task-1', title, language: 'en', created_at: now, updated_at: now }] })
