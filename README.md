...
- `2026-07-13`
  ...
  - 验证：后端完整回归 `165 passed, 6 skipped`，评估/检索专项 `33 passed`，Ruff 通过；前端 Vitest `9 passed`、Oxlint、build、Ask Paper Playwright 桌面/移动端和 Compose 配置检查通过。
- `2026-07-12`
...
| Ask-the-Paper | ✅ 单论文完成 | 持久化多轮会话、搜索、删除、Markdown/JSON 引用归档、流式恢复、Evidence 与取消/重试 |
...
报告包含 candidate Recall@20、macro Precision@6、Recall@6、MRR、Evidence coverage、拒答率、错误拒答率、平均返回数和延迟，并按语言、可回答性、章节约束及干扰类型分组。合成集只用于 CI 回归；生产阈值必须使用按论文隔离的真实验证集校准，再由冻结测试集验收。完整限制和指标口径见 [Ask Paper 离线质量评估文档](backend/evaluation/README.md)。
...
8. 切换到 **Ask Paper**，选择已完成论文并创建会话；可限定论文章节、覆盖回答语言、查看 Evidence，并按需搜索、重命名、导出或永久删除会话。
...
Ask Paper API 测试使用隔离 SQLite、显式生命周期准备、临时 state 文件和确定性 Worker fake，覆盖任务/会话隔离、字面搜索、事务级删除、Markdown/JSON 引用归档、章节校验、分页、Evidence 归属、取消/重试及 SSE 恢复协议。当前 Starlette/AnyIO 组合在线程 portal 中运行同步端点可能停滞，因此相关 HTTP 入口使用 async 路由，测试通过 ASGI transport 执行，不再排除 Ask Paper API。

前端使用 Vitest、Testing Library 和 jsdom 覆盖交互报告及 Ask Paper SSE 消费器；Playwright route fixtures 覆盖完整问答、token 去重、Evidence、搜索/无结果、两种导出、删除重选、生成期禁用、标题修改、刷新恢复、报告跳转和移动端布局。浏览器回归同时运行桌面 Chromium 与 Pixel 7 项目：
...
- [x] 为 Ask Paper 引入带最近 6 条上下文的 query rewrite 和章节过滤；改写失败时确定性降级（HyDE 与通用 metadata filtering 留待后续）。
...
- [ ] 将当前 122 条 pilot 扩展到约 200 条已复核问题；pilot validation 尚有 8 项硬门槛失败，test 未冻结，生产继续使用 BM25 且 reranker 保持 `disabled`。
- [x] 增加按任务、state 版本、章节和 Embedding 配置失效的 Worker 内 LRU 索引缓存（持久化缓存与成本统计留待后续）。
...
- `NumpyVectorStore` 仍只存在进程内；Compose 任务元数据持久化到 PostgreSQL（轻量开发与测试可使用 SQLite），但向量索引不会随任务保存。
