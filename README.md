...
- `2026-07-12`
  ...
  - 验证：五服务镜像构建、容器启动和主要功能测试通过；后端测试 `117 passed, 6 skipped`，前端 build/lint、Compose 配置与差异检查通过。
- `2026-07-11`
...
当前锁定的 Starlette 1.3 TestClient 在首个请求处仍会停滞；即使加入 `httpx2` 后问题仍可复现。CI 暂时显式排除 `backend/tests/test_api_tasks.py`，其余后端测试继续作为必过检查。前端使用 Vitest、Testing Library 和 jsdom 覆盖交互报告的目录、搜索复原、Evidence 缓存、错误重试及任务切换状态清理；Playwright 已作为开发依赖安装，浏览器级用例将在测试入口具备稳定数据注入方式后启用。
