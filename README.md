...
- `2026-07-13`
  ...
  - 验证：后端完整回归 `161 passed, 6 skipped`，评估/检索专项 `33 passed`，Ruff 通过；前端 Vitest `7 passed`、Oxlint、build、Playwright 桌面/移动端和 Compose 配置检查通过。
- `2026-07-12`
...
Ask Paper API 测试使用隔离 SQLite、显式生命周期准备、临时 state 文件和确定性 Worker fake，覆盖任务/会话隔离、章节校验、分页、Evidence 归属、取消/重试及 SSE 恢复协议。当前 Starlette/AnyIO 组合在线程 portal 中运行同步端点可能停滞，因此相关 HTTP 入口使用 async 路由，测试通过 ASGI transport 执行，不再排除 Ask Paper API。

前端使用 Vitest、Testing Library 和 jsdom 覆盖交互报告及 Ask Paper SSE 消费器；Playwright route fixtures 覆盖完整问答、token 去重、Evidence、标题修改、刷新恢复、报告跳转和移动端布局。浏览器回归同时运行桌面 Chromium 与 Pixel 7 项目：
...
- [ ] 下一产品阶段：增加会话搜索、删除及 Markdown/JSON 导出；页码范围和更精细的上下文窗口管理随后推进。
